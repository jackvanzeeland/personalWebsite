...
import { WORK_ITEMS, allWorkTags, WorkItem } from '../../data/workItems';
...
function renderCard(item: WorkItem): HTMLAnchorElement {
    ...
    if (item.tool) {
        kindRow.appendChild(el('span', 'work-kind work-kind-live', '● INTERACTIVE'));
    }
    body.append(
        kindRow,
    ...
    item.technologies.slice(0, 4).forEach((t) => tech.appendChild(el('span', 'home-tag', t)));
...
const view: View = {
    mount(root) {
        ...
        filters.addEventListener('click', (e) => {
            const chip = (e.target as Element).closest('button.work-chip') as HTMLButtonElement | null;
            if (!chip) return;
